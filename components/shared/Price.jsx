  ...
  return (
    <span className={`inline-flex items-center gap-1.5 ${className || ''}`}>
      <span>
        ...
        {mrpText && (
          <span className="ml-2 text-sm text-slate-500 line-through">{mrpText}</span>
        )}
      </span>
      ...
      {enableTooltip && (
        ...
        >
          <InfoIcon 
            size={18} 
            className="text-slate-400 hover:text-slate-600 transition-colors cursor-help flex-shrink-0" 
          />
          
          {/* Tooltip */}
          {showTooltip && (
            <span className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-800 text-white text-sm font-medium rounded-md shadow-lg whitespace-nowrap pointer-events-none">
              {tooltipText}
